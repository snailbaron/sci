...
\begin{titlepage}
	\begin{center}
		...
		\vspace{25mm}
		{\Large \textbf{\upshape{КУРСОВАЯ РАБОТА}} } \\
		\vspace{10mm}
		Тема: \\
...
\sloppy{
Обозначим $s_{lh}$ (при $l \leq h$) --- число критических классов среди подцепочки $K_l, K_{l+1}, \ldots, K_h$. Разобьём последовательность классов на группы $\mathcal{M}_1, \mathcal{M}_2, \ldots, \mathcal{M}_w$, где $w = s_{1m}$. При этом к группе $\mathcal{M}_j$ отнесём докритические классы $K_l$, для которых $s_{1l} < l$ и $l$-й критический класс. Докритические классы $K_l$, для которых $s_{1l} = w$, также отнесём к классу $\mathcal{M}_w$. Таким образом, каждая группа содержит в себе ровно один критический класс.
}
...
Для такого разбиения матрицы $A$ из \cite{zhiltsova-about-matrix} следует
\begin{theorem}
	При $t \rightarrow \infty$
	\begin{equation*}
		B^{(t)}_{lh} = U^{(h)} V^{(l)} t^{s_{lh} - 1} r^t (1 + o(1)),
	\end{equation*}
	где	$U^{(h)} = \left( U^{(h)}_I, U^{(h)}_{II}, U^{(h)}_{III} \right)$ и $V^{(l)} = \left( V^{(l)}_I, V^{(l)}_{II}, V^{(l)}_{III} \right)$ --- соответственно правый и левый собственные векторы матрицы $B_{lh}$.
	
	При этом $U^{(h)}_{II}$ --- правый собственный вектор матрицы $A_{ii}$, соответствующей $h$-му критическому классу, а $V^{(l)}_{II}$ --- левый собственный вектор матрицы $A_{jj}$, соответствующей $l$-му критическому классу.
\end{theorem}
