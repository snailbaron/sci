...
\textit{Стохастической КС-грамматикой} \cite{fu-struct} называется система $G = <V_T, V_N, R, s>$, где $V_T$ и $V_N$ --- соответственно алфавиты терминальных и нетерминальных символов (\textit{терминалов} и \textit{нетерминалов}), $s$ --- \textit{аксиома} грамматики, $R$ --- множество \textit{правил вывода}, представимое в виде $R = \cup_{i = 1}^k R_i$, где $k = |V_N|$, и $R_i$ --- множество правил вида
\begin{equation}
    r_{ij} : A_i \xrightarrow{p_{ij}} \beta_{ij}\quad{}\left( A_i \in V_N, \beta_{ij} \in (V_N \cup V_T)^* \right),
\end{equation}
где $p_{ij}$ --- вероятность применения правила $r_{ij}$, причём при фиксированном $i$ вероятности $r_{ij}$ задают вероятностное распределение на множестве $R_i$:
\begin{equation}
...
Последовательность правил грамматики $(r_1, r_2, \ldots, r_k)$ называется \textit{выводом} слова $\alpha$ (обозначается $\omega(\alpha)$), если существует последовательность слов $(\alpha_0, \alpha_1, \alpha_2, \ldots, \alpha_k)$ таких, что для любго $l$ слово $\alpha_{l}$ получается из $\alpha_{l-1}$ при замене одного из нетерминалов $\alpha_{l-1}$ согласно правилу $r_l$, и при этом $\alpha_0 = s$ и $\alpha_k = \alpha$. В случае, если каждое $\alpha_l$ получено из $\alpha_{l-1}$ заменой самого левого нетерминала в $\alpha_{l-1}$ согласно правилу $r_l$, такой вывод $\omega(\alpha)$ называют \textit{левым}.

Вероятность вывода $\omega(\alpha)$ определяется как $p(\omega(\alpha)) = p(r_1) \cdot p(r_2) \cdot \ldots \cdot p(r_k)$, где $p(r_i)$ --- вероятность применения правила $r_i$.

Каждому выводу слова $\alpha$ соответствует корневое дерево, называемое \textit{деревом вывода}. Узлам дерева соответствуют терминалы и нетерминалы. Дерево вывода $\omega(\alpha) = (r_1, \ldots, r_k)$ может быть построено следующим образом. В корень дерева помещается аксиома $s$ грамматики. Далее для правил $r_1, r_2, \ldots, r_k$ последовательно к листу дерева, соответствующему левой части правила $r_i$ приписываются буквы правой части $r_i$, образующие листья на следующем ярусе дерева. После выполнения этого действия для всех правил полученное дерево будет являться деревом вывода слова $\alpha$ в грамматике $G$, соответствующее выводу $\alpha$.

Одному дереву вывода, построенному для слову $\alpha$ могут соответствовать различные вывода этого слова. Известно, однако, что существует взаимно-однозначное соответствие между деревьями вывода слов и левыми выводами. Вероятность слова $\alpha$ в грамматике определяется как сумма вероятностей всех его левых выводов.
...
\begin{theorem}
	В принятых обозначениях при $t \rightarrow \infty$
    \begin{equation}
        B_{lh}^{(t)} = H_{lh} \cdot t^{s_{lh} - 1} r^t (1 + o(1))\quad\text{при}\quad l \neq h,
    \end{equation}
    где $H_{lh}$ не зависит от $t$, и $s_{lh}$ --- число классов с номерами из $J$ среди $K_l, K_{l+1}, \ldots, K_h$.
\end{theorem}

\section{Вероятности продолжения}

Вероятностью продолжения $Q_i(t)$ будем называть вероятность того, что дерево вывода с корнем $A_i$ будет иметь высоту не менее $t$. Для исследования этих вероятностей, по аналогии с теорией ветвящихся процессов \cite{sevast} определим \textit{производящие функции}:
\begin{equation}
\begin{split}
	F(\mathbf{s}) = &F(0, \mathbf{s}) = \sum p_{ij} s_1^{l_i^1} s_2^{l_i^2} \ldots s_{n_i}^{l_i^{n_i}} \\
	&F(t,\mathbf{s}) = F(F(t-1,\mathbf{s}))
\end{split}
\end{equation}

Раскладывая $F_i(\mathbf{s})$ в ряд Тейлора в окрестности точки $\mathbf{1} = (1,1,\ldots,1)$, получаем:
\begin{multline}
	...
	= 1 + \sum_j a^i_j (s_j - 1) + \frac{1}{2} \sum_{j,l} b^i_{jl} (s_j - 1) (s_l - 1) + \mathop{O} (|s_{k^*(\mathbf{s})} - 1|^3),
\end{multline}
где $k^*(\mathbf{s}) = \arg \max_k |s_k - 1|$

Подставляя в это выражение вместо $\mathbf{s}$ вектор $F(t,\mathbf{s})$, получаем:
\begin{multline}
	F_i(F(t,\mathbf{s})) = F_i(t+1,\mathbf{s}) = 1 + \sum_i a^i_j (F_j(t,\mathbf{s}) - 1) + \\
	+ \frac{1}{2} \sum_{j,l} b^i_{j,l} (F_j(t,\mathbf{s})) - 1) (F_l(t,\mathbf{s}) - 1) + \mathop{O} (|F_{k^*(F(t,\mathbf{s}))}(t,\mathbf{s}) - 1|^3)
\end{multline}

Нетрудно показать \cite{sevast}, что $Q_i(t) = 1 - F_i(t,\mathbf{0})$. Отсюда получаем рекуррентное соотношение для вероятностей продолжения:
\begin{equation}
	Q_i(t+1) = \sum_j a^i_j	Q_j(t) - \frac{1}{2} \sum_{j,l} b^i_{j,l} Q_j(t) Q_l(t) + O(|Q_{k^*}^3(t)|),
\end{equation}
где $Q_i(t)$ --- вероятность продолжения для деревьев высоты $t$, и $k^* = k^*(Q(t)) = \arg \min_k |Q_k(t)|$.
